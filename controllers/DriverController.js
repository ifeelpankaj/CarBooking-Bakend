...
import driverServices from "../operations/DriverServices.js";
...
export const driverVerification = async (req, res) => {
  try {
    ...
    // Use DriverService to handle document verification and uploading
    const uploadedDocuments = await driverServices.verifyAndUploadDocuments(
      req.user._id,
      documents,
      docNames
    );
...
export const getMyCabs = async (req, res, next) => {
  try {
    ...
    const cabs = await CabServices.findCabs(query, selectFields, options);
      
    res.status(200).json({
      success: true,
      length: Array.isArray(cabs) ? cabs.length : (cabs ? 1 : 0),
      cabs: Array.isArray(cabs) ? cabs : (cabs ? [cabs] : []),
    });
  } catch (error) {
    console.error(error);
...
export const getDriverBooking = async (req, res, next) => {
  try {
    ...
    const driverCabs = await CabServices.findCabs({ belongsTo: driverId });
    ...
    //   .sort({ departureDate: 1 });
      ...
      const allDriverBookings = await OrderServices.findOrders(
        {
          bookedCab: { $in: cabIds },
          bookingStatus: { $in: ['Assigning', 'Confirmed', 'Completed'] }
        },
        [
          ...
          { path: 'bookedCab', select: 'modelName cabNumber' }
        ],
        '',
        { departureDate: 1 },
        true
...
export const confirmBooking = async (req, res, next) => {
  try {
    ...
    const order = await OrderServices.updateBooking(orderId, 'confirm');
...
export const cancelBooking = async (req, res, next) => {
  try {
    ...
    const order = await OrderServices.updateBooking(orderId, 'cancel');
...
export const completeBooking = async (req, res, next) => {
  try {
    ...
    const order = await OrderServices.updateBooking(orderId, 'complete');
